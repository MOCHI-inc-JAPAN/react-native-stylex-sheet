...
以下は動的Themeによるスタイリングのベンチマークです。

[ベンチマークテスト用レポジトリ](https://github.com/tkow/react-native-stylesheet-benchmark)

![img.png](./docs/output.png)

こちらのベンチマークでReactNative標準のスタイリングが最速でない理由はダイナミックなスタイルオブジェクトをキャッシュせずに使用しているためです。もし、これらのスタイルを`StyleSheet.create`でキャッシュした場合、当然、最速、あるいは、ベンチマーク結果の上位２ライブラリと同水準の速度になります。トップ2ライブラリは理論的にほぼ同等な最速な手法でスタイリングしているため、実行環境やランダムに順位が入れ替わることがあります。

使用したライブラリのesbuild利用時のbundle size.
```txt
  ┌────────────────────────────────────────────┬───────────┬─────────────┐
